pipeline {
    ...
    stages {
        ...
        stage('Tag image') {
            steps {
                sh "docker build -t sorinnsg/hello-img:v1.1.4 ."
                ...
                docker push sorinnsg/hello-img:v1.1.4
                '''
                }

                sh "git tag v1.1.4" //image tag
                withCredentials([string(credentialsId: 'GITHUB_TOKEN', variable: 'GITHUB_TOKEN')]){
                sh "git push https://$GITHUB_TOKEN@github.com/StockZ-ProdEngineering/service.git v1.1.4" // img tag
                }
            }
        }
        stage('Deploy'){
            steps{
                sh "IMAGE_TAG=v1.1.4 docker-compose up -d hello" // img tag
            }
        }
    }
}
